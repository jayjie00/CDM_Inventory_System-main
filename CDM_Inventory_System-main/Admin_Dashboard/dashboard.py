...
def get_pdf_folder(folder_name):
    ...
    # 👉 go up to project root manually
...
class ClickableCard(QFrame):
    ...
    def __init__(self, title, color):
        ...
        self.default_geometry = None  # para sa animation
        ...
        self.label_header.setStyleSheet(f"""
    ...
    def enterEvent(self, event):
        ...
        anim.setEndValue(QRect(
            self.default_geometry.x() - 5,
            self.default_geometry.y() - 5,
            self.default_geometry.width() + 10,
            self.default_geometry.height() + 10
        ))
    ...
    def leaveEvent(self, event):
        ...
        anim.setDuration(150)
        anim.setStartValue(self.geometry())
        anim.setEndValue(self.default_geometry)
        anim.setEasingCurve(QEasingCurve.Type.OutQuad)
        anim.start()
...
class AdminDashboard(QWidget):
    ...
    def generate_history_pdf(self, row_data):
        from reportlab.lib.pagesizes import letter
        from reportlab.pdfgen import canvas
        from datetime import datetime
        import os

        folder = get_pdf_folder("Transaction_History")

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        file_path = os.path.join(folder, f"History_{timestamp}.pdf")
        ...
        c.drawString(50, y, "BORROW HISTORY")
        y -= 30

        labels = ["ID", "Student", "Items", "Purpose", "Status", "Date"]

        for label, value in zip(labels, row_data):
    ...
    def generate_inventory_pdf(self, items):
        ...
        # ✅ gamitin unified folder
        folder = get_pdf_folder("inventory")

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        file_path = os.path.join(folder, f"Inventory_{timestamp}.pdf")
        ...
        for item in items:
            text = f"{item[1]} | {item[2]} | Qty: {item[3]} | {item[5]}"
    ...
    def create_inventory_page(self):
        ...
        self.prop_id_in.setVisible(True)
        ...
        for w in [self.name_in, self.brand_in, self.qty_in, self.prop_id_in, self.cat_in, self.img_btn, add_btn, down_btn]:
    ...
    def download_inventory(self):
        try:
            items = get_all_items()
            self.generate_inventory_pdf(items)
        except Exception as e:
    ...
    def handle_update_request(self, rid, status):
        ...
        if status.upper() in ["APPROVED", "BORROWED"]:
            reqs = get_all_requests()
    ...
    def download_pdf_for_request(self, request_id):
        try:
            reqs = get_all_requests()
            this_req = next((r for r in reqs if r[0] == request_id), None)
            ...
            # Prepare data
            items_dict = json.loads(this_req[2])
            items_text = ", ".join([f"{n} (x{q})" for n, q in items_dict.items()])
            ...
            self.generate_history_pdf(row_data)
    ...
    def refresh_history(self):
        ...
        for data in reqs:
            ...
            pdf_btn.clicked.connect(lambda _, req_id=data[0]: self.download_pdf_for_request(req_id))
    ...
    def refresh_returns(self):
        ...
        for data in reqs:
            status = str(data[4]).upper()

            # ✅ FIX 1: mas flexible
            if status not in ["APPROVED", "BORROWED"]:
                continue

            items_dict = json.loads(data[2])

            # ✅ FIX 2: alisin natin yung sobrang filter
            needs_return = True
            ...
            self.ret_table.setItem(row, 0, make_item(data[0]))
            self.ret_table.setItem(row, 1, make_item(data[1]))

            txt = ", ".join([f"{n} (x{q})" for n, q in items_dict.items()])
            self.ret_table.setItem(row, 2, make_item(txt))

            self.ret_table.setItem(row, 3, make_item("BORROWED"))

            btn = QPushButton("MARK AS RETURNED")
            btn.setStyleSheet("background-color: #1B4D2E; color: white; font-weight: bold;")

            btn.clicked.connect(
                lambda ch, r_id=data[0], items=items_dict: self.handle_return(r_id, items)
            )
